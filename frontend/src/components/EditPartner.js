...
export default ({showLoading}) => {
  ...
  const [latLng, setLatLng] = useState({lat: -7.257833394913394, lng: 112.75203690996337})
  ...
  const getPartnerById = async () => {
    showLoading(true)
    const response = await axios.get(`http://localhost:5000/api/v1/partners/${id}`).then((res) => {
      setName(res.data.data.nama_mitra)
      setEmail(res.data.data.email_mitra)
      setAddress(res.data.data.drop_point)
      setPhone(res.data.data.whatsapp)
      setStatusPartner(res.data.data.status)
      setOpenHour(res.data.data.schedule.open)
      setCloseHour(res.data.data.schedule.close)
      setImage(res.data.data.image)
      setQrImage(res.data.data.qrcode)
      setTrashCategory(trashCategory.map((item) => {
        if (res.data.data.trash.includes(+item.name)) {
          item.value = true
        }
        return item 
      }))
      setDays(days.map((item) => {
        if (res.data.data.schedule.days.includes(item.name)) {
          item.value = true
        }
        return item 
      }))
      setLatitudeLongitude({lat: res.data.data.coord._latitude, lng: res.data.data.coord._longitude})
      showLoading(false)
    })
  }
  ...
  const addPartner = async (e) => {
    showLoading(true)
    e.preventDefault()
    if(name == '' || email == '' || address == '' || phone == '' || statusPartner == '' || openHour == '' || closeHour == '' || image == '' || latLng == undefined){
    ...
    try {
      ...
      formData.append('status', statusPartner)
      ...
      const { data } = await axios({
        method: 'PATCH',
        url: `http://wongdeveloper.uk/api/v1/partners/${id}`,
        data: formData,
        headers: {
              ...
              <form onSubmit={addPartner} encType="multipart/form-data">
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    id="status"
                    value={statusPartner}
                    onChange={(e) => setStatusPartner(e.target.value)}>
                    <option value="" hidden disabled>Select Status</option>
                    <option value="active" checked={statusPartner == 'active' ? true : false}>Active</option>
                    <option value="inactive" checked={statusPartner == 'inactive' ? true : false}>Inactive</option>
                    <option value="unvalidated" checked={statusPartner == 'unvalidated' ? true : false}>Unvalidated</option>
                  </select>
                </div>
                <div className="form-group">
                  ...
                  <div className="input-group">
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="1" id="plastic" onChange={(e) => updateTrashCategory(e)} checked={trashCategory[0].value} />
                      <label className="form-check-label" htmlFor="plastic">
                        Plastic 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="2" id="electronic" onChange={(e) => updateTrashCategory(e)} checked={trashCategory[1].value}/>
                      <label className="form-check-label" htmlFor="electronic">
                        Electronic Waste 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="3" id="cardboard" onChange={(e) => updateTrashCategory(e)} checked={trashCategory[2].value}/>
                ...
                <div className="form-group">
                  ...
                  <div className="input-group">
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="monday" id="monday" onChange={(e) => updateDays(e)} checked={days[0].value}/>
                      <label className="form-check-label" htmlFor="monday">
                        Monday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="tuesday" id="tuesday" onChange={(e) => updateDays(e)} checked={days[1].value}/>
                      <label className="form-check-label" htmlFor="tuesday">
                        Tuesday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="wednesday" id="wednesday" onChange={(e) => updateDays(e)} checked={days[2].value}/>
                      <label className="form-check-label" htmlFor="wednesday">
                        Wednesday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="thursday" id="thursday" onChange={(e) => updateDays(e)} checked={days[3].value}/>
                      <label className="form-check-label" htmlFor="thursday">
                        Thursday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="friday" id="friday" onChange={(e) => updateDays(e)} checked={days[4].value}/>
                      <label className="form-check-label" htmlFor="friday">
                        Friday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="saturday" id="saturday" onChange={(e) => updateDays(e)} checked={days[5].value}/>
                      <label className="form-check-label" htmlFor="saturday">
                        Saturday 
                      </label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="checkbox" value="sunday" id="sunday" onChange={(e) => updateDays(e)} checked={days[6].value} />
                ...
                <div className="form-group">
                  ...
                  <div className="input-group">
                    <input type="time" className="form-control" id="hours" onChange={(e) => setOpenHour(e.target.value)} value={openHour} />
                    <input type="time" className="form-control" id="hours" onChange={(e) => setCloseHour(e.target.value)} value={closeHour} />
                  </div>
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <div className="map-container" id="mapBox">
                    <MapContainer2 setLatitudeLongitude={setLatitudeLongitude} latLng={latLng} />
                  </div>
                </div>
